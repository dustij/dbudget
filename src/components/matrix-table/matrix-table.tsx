...
import { useRef, useState, type FC, FocusEvent, isValidElement } from "react"
...
const MatrixTable: FC<MatrixTableProps> = ({
  ...
  const [childMatrix, setChildMatrix] = useState<React.ReactNode[][]>(
    !children
      ? []
      : children.map((row, rowIndex: number) => {
          refsMatrix.current[rowIndex] = []
          // const rowChildren = (row as React.ReactElement).props
    ...
    // rowChildren.ref = (el) => {
    //   refsMatrix.current[rowIndex]![0] = el
    // }
    // console.log(rowChildren)

    // rowChildren.forEach((child, colIndex) => {
    //   child.ref = (el) => {
    //     refsMatrix.current[rowIndex]![colIndex] = el
    //   }
    // })
    // return rowChildren
    // }),
